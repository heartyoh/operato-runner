...
```bash
# 배포
helm install operato-runner ./helm-chart -f my-values.yaml

# 업그레이드
helm upgrade operato-runner ./helm-chart -f my-values.yaml
```
...
# 스테이징 환경 배포
helm install operato-runner-staging ./helm-chart -f values-staging.yaml

# 프로덕션 환경 배포
helm install operato-runner-prod ./helm-chart -f values-prod.yaml
```
