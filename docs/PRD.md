...
- stdout, stderr, exit_code, duration 등을 기록
...
- Module: name, env, path/code, created_at, version, tags
- ExecRequest: module, input_json
- ExecResult: result_json, exit_code, stderr, duration
...
4. 동적 등록 API (`/modules`) → CRUD 완성
5. 확장 실행기 (docker, conda)
6. 스케일 아웃 고려: queue 연동, 컨테이너 풀
7. 인증 및 보안 처리 → 실서비스 대응
