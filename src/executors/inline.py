...
class InlineExecutor(Executor):
    ...
    async def execute(self, request: ExecRequest) -> ExecResult:
            ...
            if module_obj:
                try:
                    ...
                    if getattr(module_obj, 'code', None):
                        code = module_obj.code
        if not code:
            # request.code 필드에서 직접 코드 가져오기
            if hasattr(request, 'code') and request.code:
                code = request.code
                print(f"DEBUG: request.code에서 코드 가져옴, 길이: {len(code)}", file=sys.stderr)
            else:
        ...
        try:
            ...
            # handler 함수가 있는지 확인하고 호출
            if "handler" in namespace and callable(namespace["handler"]):
                result_json = namespace["handler"](input_obj)
            else:
                # handler 함수가 없으면 오류
                raise Exception("handler function not found in the module code")
            
            # 결과가 None인 경우 빈 딕셔너리로 설정
