...
def create_app() -> FastAPI:
    ...
    async def list_modules(
        ...
        def is_deployed(m):
            ...
            if m.env == "inline":
                ...
                return os.path.exists(os.path.join("runtime", "module_envs", m.name, "uv"))
            elif m.env == "venv":
                return os.path.exists(os.path.join("runtime", "module_envs", m.name, "venv"))
            elif m.env == "conda":
                # conda 환경 경로 수정
                return os.path.exists(os.path.join("runtime", "module_envs", m.name, "conda"))
            elif m.env == "docker":
                ...
                try:
                    ...
                    # 전개 디렉토리도 확인
                    has_deploy_dir = os.path.exists(os.path.join("runtime", "module_envs", m.name))
                    return has_image and has_deploy_dir
                except Exception:
                    # Docker 명령어 실패 시 디렉토리만 확인
                    return os.path.exists(os.path.join("runtime", "module_envs", m.name))
            return False
    ...
    async def create_module(
        ...
        if artifact_type == "git":
            ...
            modules_dir = os.path.join("runtime", "modules", name, version)
    ...
    async def deploy_module(name: str, db: AsyncSession = Depends(get_db)):
        ...
        src_dir = os.path.join("runtime", "modules", module.name, active_version.version)
        dst_dir = os.path.join("runtime", "module_envs", module.name)
    ...
    async def undeploy_module(name: str):
        module_env_dir = os.path.abspath(os.path.join("runtime", "module_envs", name))
    ...
    async def get_deployed_info(name: str, db: AsyncSession = Depends(get_db)):
        ...
        deploy_exists = False
        if active_version:
            deploy_base_dir = os.path.join("runtime", "modules", module.name, active_version.version)
            ...
            latest_version = latest_version_result.scalars().first()
            if latest_version:
                deploy_base_dir = os.path.join("runtime", "modules", module.name, latest_version.version)
                deploy_exists = os.path.exists(deploy_base_dir)
        
        # 전개 경로 (module_envs) - 실제 실행 환경
        env_base_dir = os.path.join("runtime", "module_envs", module.name)
