...
def test_module_creation():
    m = ModuleSchema(name="testmod", env="inline", code="print('hi')", version="1.0.0", tags=["test"])
...
def test_module_validation_error():
    with pytest.raises(ValidationError):
        ModuleSchema(name=123, env=456)  # 잘못된 타입

def test_module_json_serialization():
    m = ModuleSchema(name="mod", env="venv")
    data = m.json()
    m2 = ModuleSchema.parse_raw(data)
    assert m2.name == "mod"
    assert m2.env == "venv"

def test_module_default_values():
    m = ModuleSchema(name="mod2", env="docker")
